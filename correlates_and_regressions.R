...
extended_data <- extended_data %>% 
  select(judgecnt.x, judgname, ID, judgment, opinion, issueID, year, phase, appvote, proapp,
                                  ...
                                  v2x_polyarchy_judge, polity2_judge, rank_unga_app, rank_unga_res,
                                  ideal.x, SD.x, 
...
mean_cor <- extended_data1 %>%
  select(rank_unga_judge, gdp_judge, judgname, ideal_country, SD_judge_ideal, xm_qudsest_judge) %>% 
  group_by(judgname) %>%
  mutate(median_unga = median(rank_unga_judge), 
...
cor.test(judge_cor$ideal_judge, judge_cor$median_unga, method ="spearman", exact = T)
...
med_dat <- extended_data %>% select(rideal_judge, rank_unga_judge, rank_unga_app, rank_unga_res, 
                                    year, issueID, proapp, appvote) %>%
  group_by(issueID) %>% 
  mutate(median_panel = median(rideal_judge, ordered_low = T)) %>% 
  ungroup() %>%
  select(year, median_panel, issueID,proapp, appvote, rank_unga_judge, rank_unga_app, rank_unga_res) %>% 
...
p<-ggplot(med_dat, aes(x=year, y=median_panel, label = judgname)) + 
...
mod1 <- glm(proapp ~ rank_unga_judge*rank_unga_app, 
            family = binomial(link = "logit"), data = med_dat)
summary(mod1)
...
extended_data2 <- extended_data %>%      #impute median and mean values for missing unga and gdp values 
  group_by(judgecnt) %>%                 #by judge ID
  mutate(ideal_unga = ifelse(is.na(rank_unga_judge), mean(rank_unga_judge, na.rm = TRUE), ideal_unga_judge),
         gdp_judge = ifelse(is.na(gdp_judge), mean(gdp_judge, na.rm = TRUE), gdp_judge))
...
mean_cor_cnt <- extended_data2 %>% # take mean of unga country and gdp
  ...
  group_by(judgecnt) %>%
  mutate(median_unga_country = median(ideal_unga, 
...
m1 <- lm(ideal_country ~ median_unga_country +
           jud_tradition,
           #mean_gdp,
           #mean_dem_country,
...
cor.test(mean_cor_cnt$ideal_country, 
         mean_cor_cnt$median_unga_country, method = "spearman", exact = T)
...
model2a <-lm(as.numeric(appvote) ~ rank_unga_judge +
             rank_unga_judge*rank_unga_res +
             rank_unga_judge*rank_unga_app,
           data = data1)
summary(model2a)

model3a <-lm(as.numeric(appvote) ~ rank_unga_judge +
               rank_unga_judge*rank_unga_res +
               rank_unga_judge*rank_unga_app +
...
model4a <-lm(as.numeric(appvote) ~ rank_unga_judge + 
               rank_unga_judge*rank_unga_res +
             rank_unga_judge*rank_unga_app +
...
model5a <-lm(as.numeric(appvote) ~ rank_unga_judge +
               rank_unga_judge*rank_unga_res +
               rank_unga_judge*rank_unga_app +
...
model6a <-lm(as.numeric(appvote) ~ rank_unga_judge +
               rank_unga_judge*rank_unga_res +
               rank_unga_judge*rank_unga_app +
               app_jud_match + res_jud_match + decade + 
...
model2<-glm(as.factor(appvote) ~ rank_unga_judge + 
              rank_unga_judge*rank_unga_res + 
              rank_unga_judge*rank_unga_app,
...
model3<-glm(as.factor(appvote) ~ rank_unga_judge + 
              rank_unga_judge*rank_unga_res +
              rank_unga_judge*rank_unga_app +
              app_jud_match + res_jud_match,
            family = binomial(link = "logit"),
...
model4<-glm(as.factor(appvote) ~ rank_unga_judge + 
              rank_unga_judge*rank_unga_res +
              rank_unga_judge*rank_unga_app +
              app_jud_match + res_jud_match + decade,
            family = binomial(link = "logit"),
...
model5<-glm(as.factor(appvote) ~ rank_unga_judge + 
              rank_unga_judge*rank_unga_res +
              rank_unga_judge*rank_unga_app +
              app_jud_match + res_jud_match + decade + judgment,
            family = binomial(link = "logit"),
...
model6<-glm(as.factor(appvote) ~ rank_unga_judge + 
              rank_unga_judge*rank_unga_res +
              rank_unga_judge*rank_unga_app +
              app_jud_match + res_jud_match + decade + judgment +
              jud_tradition,
...
model7<-glm(as.factor(appvote) ~ rank_unga_judge + 
              rank_unga_judge*rank_unga_res +
              rank_unga_judge*rank_unga_app +
...
library(stargazer)
stargazer(model2a,model3a,model4a,model5a,model6a,
          type = "latex", digits = 4, digits.extra = 3,
          omit=c("decade","app_jud_match", "res_jud_match", "judgment", "judgname"),title="",
          model.names = F,covariate.labels=c("Judge UNGA Rank",
                                             "Respondent UNGA Rank",
                                             "Applicant UNGA Rank",
                                             "Common Law", "Islamic Law",
                                             "Socialist Law",
                                             "Judge UNGA Rank*Respondent UNGA Rank",
                                             "Judge UNGA Rank*Applicant UNGA Rank"),
